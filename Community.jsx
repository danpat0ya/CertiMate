...
const Community = () => {
  ...
  const defaultNoticePosts = [
    { id: 'notice-3', category: '공지사항', title: '📁 [6월 1주차] 취준생 필독! 이번주 합격 스펙&취준 꿀팁 LIST', content: '이번 주 꼭 확인해야 할 합격 스펙과 꿀팁을 정리해 드립니다!\n\n1. 어학 점수 커트라인\n2. 필수 자격증 리스트\n3. 면접 빈출 질문', writer: 'CertiMate', date: '2026.06.05', views: 262, recommendations: 12, scraps: 0, comments: 1, isNotice: true, icon: <Folder className="text-yellow-500" size={16} />, imageUrl: null, replyList: [{ id: 1, writer: '취준파이터', date: '2026.06.05', content: '좋은 정보 감사합니다!' }] },
    { id: 'notice-2', category: '자유게시판', title: '🍽️ 취뽀한 날 당일 저녁 뭐 먹고 싶어? 댓글 달고 치킨 받자!', content: '드디어 취업에 성공한 여러분! 가장 먼저 먹고 싶은 음식은 무엇인가요?\n댓글로 남겨주시면 추첨을 통해 5분께 치킨 기프티콘을 쏩니다!', writer: 'CertiMate', date: '2026.06.04', views: 258, recommendations: 8, scraps: 0, comments: 0, isNotice: true, icon: <ImageIcon className="text-gray-400" size={16} />, imageUrl: null, replyList: [] },
    { id: 'notice-1', category: 'BEST 취준', title: '🏆 [6월 1주차] 취준생이 꼭 봐야할 인턴 공고 스펙 총정리', content: '현재 모집 중인 주요 기업 인턴 공고와 합격자 평균 스펙을 모았습니다. 놓치지 말고 지원하세요!', writer: 'CertiMate', date: '2026.06.04', views: 997, recommendations: 45, scraps: 5, comments: 0, isNotice: true, icon: <Trophy className="text-yellow-500" size={16} />, imageUrl: null, replyList: [] },
  ];
  ...
  const fetchPostsFromDB = async () => {
    try {
      ...
      const formattedPosts = data.map(post => ({
        ...
        writer: post.writer || 'NV_51630***',
        date: post.date,
        views: post.views || 0,
        recommendations: post.recommendations || 0,
        scraps: 0,
        comments: post.comments || (post.replyList ? post.replyList.length : 0),
        isNotice: false,
        icon: post.imageUrl ? <ImageIcon className="text-[#007BFF]" size={16} /> : null,
        imageUrl: post.imageUrl ? `http://localhost:8080${post.imageUrl}` : null,
        replyList: post.replyList || []
      }));

      // DB 게시글 + 고정 공지사항 병합
      setAllPosts([...formattedPosts, ...defaultNoticePosts]);
    } catch (error) {
      console.error("DB 게시글 조회 실패:", error);
    }
  };

  // 새로고침 시 / 페이지 진입 시 DB에서 데이터 자동 로드
  useEffect(() => {
    fetchPostsFromDB();
  }, []);
  ...
  // 추천(좋아요) 게시물 ID 목록 (로컬스토리지에 저장하여 인당 1회 누르기 제한)
  ...
  const handleCreatePost = async (e) => {
    ...
    try {
      ...
      apiFormData.append('nickname', 'NV_51630***'); // 작성자 별명
      if (attachedImage) {
        apiFormData.append('image', attachedImage); // 사진 파일 첨부
      }

      // Spring Boot comty REST Controller 로 요청 전송
      ...
      const newPostObj = {
        ...
        recommendations: savedData.recommendations || 0,
        scraps: 0,
        comments: 0,
        isNotice: false,
        icon: savedData.imageUrl ? <ImageIcon className="text-[#007BFF]" size={16} /> : null,
        imageUrl: savedData.imageUrl ? `http://localhost:8080${savedData.imageUrl}` : null,
        replyList: []
      };
  ...
  // 게시글 클릭 시 상세 화면으로 이동 (DB 조회수 증가 및 댓글 로드)
  const handlePostClick = async (post) => {
    if (typeof post.id === 'number') {
      try {
        const response = await fetch(`http://localhost:8080/api/community/posts/${post.id}`);
        if (response.ok) {
          const detailData = await response.json();
          const formattedDetail = {
            id: detailData.id,
            category: detailData.category || '자유게시판',
            title: detailData.title,
            content: detailData.content,
            writer: detailData.writer || 'NV_51630***',
            date: detailData.date,
            views: detailData.views || 0,
            recommendations: detailData.recommendations || 0,
            scraps: 0,
            comments: detailData.comments || (detailData.replyList ? detailData.replyList.length : 0),
            isNotice: false,
            imageUrl: detailData.imageUrl ? `http://localhost:8080${detailData.imageUrl}` : null,
            replyList: detailData.replyList || []
          };
          setSelectedPost(formattedDetail);
          setAllPosts(prev => prev.map(p => p.id === post.id ? formattedDetail : p));
          return;
        }
      } catch (err) {
        console.error("상세 게시글 DB 조회 실패:", err);
      }
    }
    const updatedPost = { ...post, views: post.views + 1 };
    setAllPosts(allPosts.map(p => p.id === post.id ? updatedPost : p));
  ...
  // 댓글 등록 함수 (Spring API 연동 - comments DB 저장)
  const handleAddComment = async () => {
    ...
    try {
      ...
      const updatedPost = {
        ...
        replyList: [...(selectedPost.replyList || []), newComment],
      };
      ...
      const fallbackComment = { id: Date.now(), writer: 'NV_51630***', date: formattedDate, content: commentInput };
      const updatedPost = { ...selectedPost, comments: (selectedPost.comments || 0) + 1, replyList: [...(selectedPost.replyList || []), fallbackComment] };
      setSelectedPost(updatedPost);
      setCommentInput('');
    }
  };

  // 게시글 추천 (좋아요) 처리 함수 (DB 연동 & 1인 1회 제한)
  const handleRecommend = async (postId) => {
    if (!postId) return;

    // 인당 한 게시물에는 1번씩만 누를 수 있게 제한
    ...
    if (typeof postId === 'number') {
      try {
        const response = await fetch(`http://localhost:8080/api/community/posts/${postId}/recommend`, {
          method: 'POST',
        });
        if (response.ok) {
          const detailData = await response.json();
          const newRecommendations = detailData.recommendations;

          setSelectedPost(prev => (prev && prev.id === postId ? { ...prev, recommendations: newRecommendations } : prev));
          setAllPosts(prev => prev.map(p => p.id === postId ? { ...p, recommendations: newRecommendations } : p));

          const nextLiked = [...likedPosts, postId];
          setLikedPosts(nextLiked);
          localStorage.setItem('likedPosts', JSON.stringify(nextLiked));
          return;
        }
      } catch (err) {
        console.error("추천 DB 업데이트 실패:", err);
      }
    }

    // 폴백 (기본 공지사항 등)
    setSelectedPost(prev => (prev && prev.id === postId ? { ...prev, recommendations: (prev.recommendations || 0) + 1 } : prev));
    setAllPosts(prev => prev.map(p => p.id === postId ? { ...p, recommendations: (p.recommendations || 0) + 1 } : p));
    ...
    return (
      <main className="w-full max-w-5xl mx-auto px-6 py-10 bg-white font-sans text-gray-800 antialiased min-h-screen">
        ...
        <div className="flex justify-between items-center border-t border-gray-200 pt-6 mb-10">
          ...
          <button 
            onClick={() => handleRecommend(selectedPost.id)}
            className={`flex items-center px-4 py-2 border rounded-lg text-sm font-bold transition ${
              likedPosts.includes(selectedPost.id)
                ? 'border-[#007BFF] text-[#007BFF]'
                : 'border-gray-300 text-gray-600 hover:text-[#007BFF] hover:border-[#007BFF]'
            }`}
          >
            ...
            {bestPosts.map((post, index) => (
              ...
              >
                <div className="flex items-center space-x-3 overflow-hidden">
                  ...
                  <p className="text-sm font-semibold text-slate-800 group-hover:text-[#007BFF] transition truncate">
                    {post.title}
                  </p>
                </div>
                <span className="text-sm font-semibold text-[#DC3545] ml-2">
                  ({post.comments || 0})
                </span>
              </div>
            ))}
